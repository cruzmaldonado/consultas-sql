-- como crear y rellenar las tablas del ejercicio
...
-- 1) comando  join para hacer una union de clients y accounts 


select clients.id as id_client, clients.first_name as nombre, clients.last_name as apellido, clients.email , accounts.account_no as cuenta, accounts.balance as saldo from clients left join accounts on clients.id= accounts.client_id;


-- 2) Seleccionar todas las cuentas y el tipo de cuenta


select accounts.account_no as cuenta, accounts.client_id as "id cliente", account_types.name as "tipo cuenta" 
from accounts right join account_types on accounts.type =account_types.id;


-- 3) Selecciona todas las transacciones realizadas


select transactions.id as transaccion, transactions.amount as monto, transactions_types.name as "tipo de transaccion"
from transactions inner join transactions_types on transactions.transaction_type= transactions_types.id;


-- 4)Selecciona todas las transacciones realizadas
...
-- 5) creando vista por cada consulta


--vista 1
create view view_registro_usuario as 
select clients.id as id_client, clients.first_name as nombre, clients.last_name as apellido, clients.email , accounts.account_no as cuenta, 
accounts.balance as saldo from clients left join accounts on clients.id= accounts.client_id;

--vista 2
create view view_registro_cuenta as
select accounts.account_no as cuenta, accounts.client_id as "id cliente", account_types.name as "tipo cuenta" 
from accounts right join account_types on accounts.type =account_types.id;
...
--vista 3
create view view_registro_transacciones as
select transactions.id as transaccion, transactions.amount as monto, transactions_types.name as "tipo de transaccion"
from transactions inner join transactions_types on transactions.transaction_type= transactions_types.id;

--vista 4
create view view_registro_todas_transacciones as
select transactions.id as transaccion, transactions.amount as monto, transactions_types.name as "tipo de transaccion"
from transactions right join transactions_types on transactions.transaction_type= transactions_types.id;
